<template>
	<view>
		...
		<view class="exam-content" style="margin-top: -15px;">
			<view class="bottom">
				<view class="left">
					<view class="pictrue">
		...
		<view class="formal">
			<view class="header">
				<view class="title">村务通</view>
			</view>
			<view class="list">
				<view class="item" @tap="goNext(index)" v-for="(item,index) in list3">
					<image :src="item.img"></image>
					<view class="text">{{item.title}}</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			record() {
				console.log('to 百宝乡')
				uni.navigateTo({
					url: '/pages/baibaoxiang/index/index'
				})
			},
			secret() {
				uni.showToast({
...
<style lang="scss" scoped>
	...
	.header.all {
		background: linear-gradient(180deg, #4687FF, #79ACFF, #FFFFFF);
		...
		.bottom {
			...
			.right {
				...
				.item {
					...
					padding-left: 10px;
					border-top: 1px solid #dedede;
					background: url('@/static/index/jilu.png') 30px center/20px 20px no-repeat;
				...
				.item~.item {
					margin-top: 10px;
					background: url('@/static/index/mijuan.png') 30px center/20px 20px no-repeat;
				}
			}
		}
	}
	...
	.formal {
		...
		.bottom {
			display: flex;
			height: 100px;
			background: url('@/static/index/formal.png') 250px center/75px 75px no-repeat;
